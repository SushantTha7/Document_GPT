...
vector_store = FAISS.load_local(faiss_index_path, hf_embedding_model)
...
def chat():
    ...
    if docs:
        ...
        metadata = " ".join([f"{doc.metadata['source']}+{doc.metadata['page']}" for doc in docs])
        print("Generated context from filtered documents.", docs[0].metadata)
    else:
        context = "missing"
        question = "I don't know what kind of document you have"
        metadata = "empty"
